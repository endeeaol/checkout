...
{block name='address_selector_blocks'}
	{foreach $addresses as $address}
		<article class="js-address-item address-item col-12 col-md-6 checkout-option-block address-selector-block {if $address.id == $selected} selected{/if}" id="{$name|classname}-address-{$address.id}">
			<input type="radio" name="{$name}" value="{$address.id}" id="r-{$name|classname}-address-{$address.id}" class="custom-control-input" {if $address.id == $selected}checked{/if}>
			<label for="r-{$name|classname}-address-{$address.id}" class="card mb-0 cursor-pointer h-100">
				<div class="address__header card-header"> {$address.alias} </div>
				<div class="card-body address__body">{$address.formatted nofilter}</div>
				{if $interactive}
